...
- CBS and NPR feeds can also be crowded with low-salience but fresh items such as transcripts, show rundowns (for example "Sunday Morning"), archive reposts, teaser/promotional entries, and soft-feature pieces. For a global top-news digest, explicitly filter these before ranking significance; publication recency alone is not enough.
- NPR World can also skew toward interview-format analysis pieces and culture/travel features even when they are fresh and well-reported. Treat these as secondary candidates: use them only when they add a distinct geopolitical/economic angle not already covered by harder news, and do not let them crowd out direct spot-news reporting from AP/CBS.
...
- Do not use shell forms like `curl ... | python3 - <<'PY' ... PY` to parse article HTML. In that pattern, Python consumes stdin for the here-doc script instead of the `curl` output, so the parser sees empty input and `curl` often reports broken-pipe errors. If you need Python-based parsing, either save the response to a temp file first, use `python3 -c '...'` so stdin remains available for the pipe, or run one self-contained Python script that invokes `curl` itself.
...
- In at least one run, the same AP article also exposed JSON-LD `"datePublished":"...Z"` while the `article:published_time` meta tag was timezone-naive. For 24-hour filtering, prefer the timezone-aware JSON-LD `datePublished` when available and use the naive meta tag only as a fallback.
...
- CBS page HTML can contain incidental `404` / `Page Not Found` strings inside scripts, templates, or navigation even when the article itself loads correctly. Do not classify a CBS page as dead with a naive substring search alone; first check the HTTP status and then confirm with structured metadata such as `og:title`, `<title>`, or JSON-LD `datePublished`.
- CBS feed links must be used exactly as published. In practice, a guessed article slug like `/news/.../` may fail or 404 while the exact RSS link (often a `/live-updates/.../` URL) resolves and exposes valid metadata. Do not reconstruct CBS URLs from headlines; verify the literal feed URL.
- CBS live-update pages are acceptable primary sources for rolling major stories when they expose page-level metadata. In practice, these pages reliably expose JSON-LD `datePublished` and `dateModified`; prefer `datePublished` for the 24-hour filter and use `dateModified` only as update context.
...
- AP availability can degrade further than the section-page probe suggests: section pages may download successfully while every high-priority extracted article URL in the first batch returns `error code: 1015`. When this happens, stop spending time on AP article verification and pivot to RSS-backed sources rather than burning the cron window.
...
- In some runs, browser automation reachability varies sharply even within AP: `world-news` may open cleanly and show fresh lead items plus relative freshness labels, while `business` and `science` time out. If one AP section snapshot works, use it opportunistically as a discovery layer instead of insisting on all sections behaving the same.
- In some runs, parsing full AP section HTML with Python regex over multi-megabyte responses was surprisingly slow or timed out repeatedly even when simple `curl -I` and targeted metadata extraction worked. When AP section scraping stalls, use browser automation on the section page as a discovery layer: the accessibility snapshot can surface the lead headlines plus relative freshness labels (for example `3 mins ago`, `37 mins ago`) much faster than bulk HTML parsing. Then fetch only the selected article URLs with `curl` and extract `og:title` / `og:description` / `article:published_time`.
- When extracting AP/CBS/NPR page metadata with regex, favor direct `curl -L --compressed` body fetches over mixed header/body capture unless you really need HTTP headers. Parsing `stdout` from a combined `-D -` response is easy to get wrong, while the plain decompressed body reliably exposes `og:title`, `og:description`, and JSON-LD `datePublished`.
- CBS article fetches may exit with `curl` timeout code 28 after transferring hundreds of KB, but the partial HTML body can still already contain usable `og:title`, `og:description`, and JSON-LD `datePublished`. Do not discard such pages automatically just because `curl` returned non-zero: inspect the captured stdout first, and if the metadata is present and the page is clearly the intended article rather than an error page, it is still a valid verification source.
- AP article fetches can intermittently return a tiny body (for example ~16 bytes) even with HTTP 200 when using a minimal user agent such as plain `Mozilla/5.0`. In a later retry, sending a more browser-like Chrome user agent (`Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/123.0.0.0 Safari/537.36`) returned the full HTML and exposed the expected metadata. If AP article verification suddenly starts coming back empty/tiny while section pages still load, retry the same URL with a fuller browser UA before discarding the story.
- In another run, `browser_navigate` to `https://apnews.com/world-news` timed out after 60s even though `curl` to the same URL returned a full multi-megabyte page quickly. Treat AP browser automation as optional discovery help only; if browser navigation hangs, fall back immediately to shell `curl`/RSS instead of spending more time on the browser path.
- AP can also fail at the individual-article level in a second, different way: some extracted article URLs return a small generic shell page (around 7 KB) with no `og:title` / `og:description` / `datePublished`, while neighboring article URLs from the same section return full article HTML. When this happens, classify that URL as unverifiable and skip it quickly rather than retrying many times.
- AP hub coverage is inconsistent by path: in one run `https://apnews.com/hub/ap-top-news` and `https://apnews.com/hub/world-news` exposed many article links, while `https://apnews.com/hub/business` exposed zero extractable article URLs. Do not assume every AP hub is populated just because another hub works; probe several paths and keep RSS sources ready.
- In one later run, AP section pages degraded further: `world-news` still exposed many `/article/...` links, but `business`, `science`, and `politics` returned zero usable article URLs while the AP homepage (`https://apnews.com/`) still exposed a large, diverse pool of fresh article links. Treat the AP homepage as an important fallback discovery surface when section/hub pages go sparse or collapse.
- AP article verification can also be highly nondeterministic across requests: the same article URL may return a small generic shell page on one fetch and full metadata-rich HTML on a retry a second later. Before discarding an otherwise promising AP article as unverifiable, retry it a small number of times with the same browser-like user agent.
- CBS video pages can still be valid verification sources for major spot-news items when they return HTTP 200 and expose `og:title`, `og:description`, and JSON-LD `datePublished`. For example, a severe-weather clip page carried usable metadata and an exact publication time. Do not reject a CBS candidate solely because the URL path is `/video/`; evaluate whether it is a substantive hard-news item rather than a promo or recap.
- Some CBS article pages can expose broken or misleading metadata even when the page is reachable: for example, `og:title` may be truncated and `og:description` may describe a different but nearby story block. When CBS page-level metadata looks inconsistent with the feed title/link, do not trust that page for the digest; either fall back to a better-verified source for the topic or skip the item entirely.
