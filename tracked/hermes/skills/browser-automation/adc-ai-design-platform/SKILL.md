...
4. **AI生图任务启动后，要轮询页面文本而不是盲等。**
   ...
   - **不要在轮询中周期性 `page.reload()`**：这会打断 Vue SPA 中正在进行的生成状态，导致脚本一直等不到结果。

5. **登录页选择器要以当前真实 placeholder 为准。**
   - 实测登录页输入框当前是：`输入账号`、`输入密码`。
   - 不要写死旧占位文案（例如 `请输入手机号`），否则 Playwright 模板会直接在登录阶段失效。
